...
import { v1 } from "uuid";
import { tasksActions, TasksForTodolistType, tasksReducer } from "features/reducers/tasks-reducer";
...
test("correct task should be deleted from correct array", () => {
  ...
  let result = tasksReducer(startState, tasksActions.removeTaskAC({ taskId: taskId1, todolistId: todolistId1 }));
...
test("should be add correct task", () => {
  ...
  let result = tasksReducer(
    startState,
    tasksActions.addTaskAC({
      task: {
        id: "taskId",
        todoListId: todolistId1,
        status: 0,
...
test("should be changed title correct task", () => {
  ...
  let result = tasksReducer(
    startState,
    tasksActions.updateTaskAC({ todolistId: todolistId1, taskId, model: { title: "updatedTitle" } }),
  );
...
test("should be changed status correct task", () => {
  ...
  let result = tasksReducer(startState, tasksActions.updateTaskAC({ todolistId: todolistId1, taskId, model }));
...
test("empty array should be added when we set todolists", () => {
  const action = todolistsActions.setTodolistsAC({ todolists: [{ id: "1", title: "title 1", order: 0, addedDate: "" }] });
...
test("tasks should be added for todolist", () => {
  const action = tasksActions.setTasksAC({
    tasks: [
      {
        id: v1(),
        title: "Js",
